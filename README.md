...
Но все равно, драматически место не увеличить, а головняка в пересборку образа добавится.
...
Сначала сдуваем оригинальную фулфлешъ.

Втыкаем в программатор. И аккураттненько, с дублированием куда-ньть, сливаем с нее полный дамп.

Ценность дампа - ART раздел, в котором содержатся калибровачные данные для wifi. Без этих данных не будет заводиться wifi.

Есть еще mac-адрес и WDS-pin, но это можно и руками поправить-переделать. 

Главное - НЕ ПОТЕРЯЙТЕ ART оригинальный.



opwnWRT (у моих tp-linkов - точно) использует DTS - linux device tree для "detecting undetecteable hardware"
Например, для разделения флешки на разделы. Она - не диск, там не все так просто.
Посмотреть разблюдовку разделов мы будем в сырцах openWRT.

Считать будем будем блоками по 64к.


**первые 128кб - два блока - загрузчик**
...
Для archer_C20 оказалось - не последний блок.
...
**для debian-ubuntu**
	...
	xsltproc zlib1g-dev libcap-dev
