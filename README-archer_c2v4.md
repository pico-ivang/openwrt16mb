...
======================================
#### с последующей установкой OpenWRT
...
Тут будем курочить TP-Link Archer C20 v4

Ничо такой. Жаль нет USB.  
Но двухдиапазонник. Насчет mimo - не знаю.  

Есть вывод UART. Про JTAG - не знаю, надо уточнить.  
...
Смотрим на описание этого аппарата в базе openWRT.  
MediaTek MT7628AN.  
это не Atheros. Хотя и близко (atheros продался mediatek`у)    
Но в терминах openWRT - теперь это target ramips-mt76x8, а не ath79, как были wdr3600/4300.

Спойлер - у него не ART, а CONFIG раздел.  
И лежит чуть в другом месте, по другому смещению.

Искаропки на archer c20 v4 идет чип памяти (внутренняя флешка) 25q64 на 8 мбайт в корпусе sop-8.  
Маловато.
...
16Мбайт - не сильно много.   
В идеале бы конечно еще больше - тут кашу маслом не испортить.   
...
Пока отложим идею в сторону.   
Все равно, USB-флешка на 32гига за шапку сухарей - гораздо более экономически целесообразно.
...
Часть аппаратная
================
Будем делать штатную 8Мб флешку на 16Мб + usb-флешка для потенциального переноса на нее overlayFS.

Сначала сдуваем с платы маршрутизатора оригинальную фулфлешъ.  
Втыкаем в программатор. И аккуратненько, с последующим дублированием куда-ньть, сливаем с нее полный дамп.
...
Для archer_C20 оказалось, что раздел ART начинается раньше на несколько 64л блоков.
Поэтому тут лучше в каждом конкретном случае лучше смотреть DTS-файл для исходного маршрутки
...
мы пропускаем 127 блоков по 64к, и dd-шим весь оставшийся один блок №128 размером 64к
...
ART и фуллфлешЪ - на флешку и в сервант за стекло, в хрустальную вазочку для аджики (не все поймут, немногие вспомнят).
...
u-boot с патчем на поддержку 16мб. Либо китайский breed тоже подойдет.
...
Но мы запустимся пока так. А потом соберем openWrt из сырцов с правильной картой разделов под наши нужды. (DTS компилится на этапе make из сырцов - поэтому рыками поправить пока никак)
...
есть вариантец сборщика  ( *надо бы сюда вставить ссылку на скриптец* )
...
openWRT версии 22 хочет чуть другой набор софта
...
А вот это нужно сделать чтобы сработала проверка на qosify в openwrt-22
...
кстати. там появилась уже 22.03, с ядром посвежее, но мы пока по классике
...
нужно поменять мап разделов в образе флешки на версию на 16Мб.

	nano openwrt/target/linux/ramips/dts/mt7628an_tplink_archer-c20-v4.dts

Он, оказывается, использует файл

	nano openwrt/target/linux/ath79/dts/mt7628an_tplink_8m.dtsi

в нем ищем  блок, где описано, до какого адреса будет раздел с firmware
и откуда начнется config
...
что мы тут видим:  
первые два 64k блока - boot раздел.  
далее идет прошивка
с адреса 0x7c0000 один блок - config. вероятно, это типа ART.    
далее - три блока некий factory.    
хз, может, конфиги для factory reset, может, еще кто.      

надо двинуть   

получается, что в оригинале
от 0x0     размером 0x20000   до 0x20000 - boot  
от 0x20000 размером 0x7a0000  до 0x7c0000 - прошивка  
от 0x7c0000 размером 0x10000  до 0x7d0000 - конфиг  
от 0x7d0000 размером 0x30000  до 0x800000 - factory  

добавляем 0x80000 к размеру firmware, то получим:  

начало прошивки 0x20000  
размер прошивки 0x7a0000 + 0x800000 -> 0xfa0000  
конец прошивки 0x20000 + 0x7a0000 + 0x800000-> 0xfc0000  
начало конфига 0xfc0000
конец конфига 0xfc0000 + 0x10000- > 0xfd0000  
начало factory 0xfd0000  
конец factory 0xfd0000 + 0x30000 -> 0x1000000  

пишем  
**размер firmware - 0xfa0000  
начало config   - 0xfc0000  
начало factory  - 0xfd0000**
